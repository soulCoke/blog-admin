...
(function ($) {
    $.extend({
        ...
        // 操作封装处理
        operate: {
    		ajax: function(url, data, callback, option){
				var config = {
					url: url,
					type: option==undefined ||  option.type==undefined ? "post" : option.type,
					dataType: option==undefined || option.dataType==undefined ? "json" : option.dataType,
					async: option==undefined || option.async==undefined ? true : option.async,
					data: data,
					beforeSend: function () {
						if(option !=undefined && !option.loadingFlag ){
							//不弹loading
						}else{
			...
			// 保存信息 有文件的 刷新表格
			saveWithFile: function(url, data, callback) {
				var config = {
					url: url,
					type: "post",
					dataType: "json",
